 ...
 */
/*
 * The plugin is instantiated once per MDX document, but the bibliography files are
 * identical across documents; cache the parsed result so citation-js runs once per path set
 */
const citationsCache = new Map();
...
function loadCitations(citationPaths) {
  ...
  if (citationsCache.has(cacheKey)) return citationsCache.get(cacheKey);
  ...
  citationsCache.set(cacheKey, citations);
