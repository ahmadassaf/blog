  ...
  return (
    <div>
      { !hideTitle && <div className='pt-10'>
        <h1 className={ cn('pb-6 text-4xl font-extrabold leading-9 tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl sm:leading-10 md:text-4xl md:leading-14', className) }>
          Featured Posts
        </h1>
      </div> }
            ...
            {displayPosts.map((post) => (
              <article key={ post.slug } className='py-8'>
                <div className='group relative'>
                  ...
                  <p className='mt-4 text-md leading-6 text-gray-600 dark:text-gray-300'>{post.summary}</p>
                </div>
              </article>
            ))}
          </div>
