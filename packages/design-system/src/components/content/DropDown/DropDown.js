...
const MenuDropDown = ({ name, menuDropDownOpen, setMenuDropDownOpen }) => {
  ...
  return (
    <button ref={ ref } type='button' className={ `flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-medium leading-7 text-gray-900 hover:bg-blue-50 dark:text-white dark:hover:bg-gray-700 cursor-pointer focus:outline-none ${mounted && menuDropDownOpen ? 'bg-blue-50 dark:bg-gray-700 text-blue-600 dark:text-blue-400' : ''}` } aria-controls='disclosure-1' aria-expanded={ mounted ? menuDropDownOpen : false } onClick={ handlemenuDropDownOpen }>
      { name }
