...
export default function rehypeSimpleCitations(options = {}) {
  ...
  return (tree, file) => {
    ...
    visit(tree, 'text', (node, index, parent) => {
      ...
      // Safer regex matching to avoid undefined issues
      try {
        let match;

        while ((match = citationGroupPattern.exec(node.value)) !== null)
          matches.push(match);

      } catch (error) {
        console.warn('Citation regex matching failed:', error);
      ...
      for (const match of matches) {
        ...
        const citationContent = match.groups?.citations || match[1];
        const matchIndex = match.index;

        // Skip if citationContent is undefined or empty
        if (!citationContent || typeof citationContent !== 'string') {
          console.warn('Skipping invalid citation content:', citationContent);
          continue;
        }
            ...
            'children': [
              {
                ...
                'properties': {
                  'ariaLabel': citationKeys.length === 1 ? `Reference ${citationNumbers[0]}` : `References ${citationNumbers.join(', ')}`,
                  ...
                  'href': `#citation-${citationKeys[0]}`,
                  'id': citationId,
                  'title': citationKeys.length === 1 ? 'View reference' : 'View references'
                },
                'tagName': 'a',
                'type': 'element'
              }
