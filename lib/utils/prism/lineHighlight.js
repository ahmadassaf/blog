...
(function lineHighlight() {
   ...
   */
  const isLineHeightRounded = (function isLineHeightRounded() {
    let res;

    return function _isLineHeightRounded() {
  ...
  function isActiveFor(pre) {
    if (!pre || !/pre/i.test(pre.nodeName)) return false;

    if (pre.hasAttribute('data-line')) return true;

    if (pre.id && Prism.util.isActive(pre, LINKABLE_LINE_NUMBERS_CLASS)) return true;
  ...
  Prism.plugins.lineHighlight = {
    ...
    'highlightLines': function highlightLines(pre, lines, classes) {
      const highlightedLines = typeof lines === 'string' ? lines : (pre.getAttribute('data-line') || '');

      const ranges = highlightedLines.replace(/\s+/g, '').split(',').filter(Boolean);
      ...
      ranges.forEach((currentRange) => {
        ...
        if (hasLineNumbers && Prism.plugins.lineNumbers) {
          ...
          mutateActions.push(() => {
            ...
            if (end > start) line.setAttribute('data-end', String(end));
      ...
      if (hasLineNumbers && Prism.util.isActive(pre, LINKABLE_LINE_NUMBERS_CLASS) && id) {
        ...
        if (!hasClass(pre, LINKABLE_LINE_NUMBERS_CLASS)) mutateActions.push(() => {
        ...
        $$('.line-numbers-rows > span', pre).forEach((lineSpan, i) => {
          ...
          lineSpan.onclick = function onclick() {
  ...
  function applyHash() {
    ...
    if (scrollIntoView) document.querySelector('.temporary.line-highlight').scrollIntoView();
  ...
  Prism.hooks.add('before-sanity-check', (env) => {
    ...
    if (num && /^(?: \n)+$/.test(env.code.slice(-num))) env.code = env.code.slice(0, -num);
  ...
  window.addEventListener('resize', () => {
    const actions = $$('pre')
      .filter(isActiveFor)
      .map((pre) => Prism.plugins.lineHighlight.highlightLines(pre));
