...
const getFileIcon = (name, isFolder = false, isOpen = false) => {
  if (isFolder)
    return isOpen ? <VscFolderOpened className='text-blue-500' /> : <VscFolder className='text-blue-400' />;
  ...
  if ([ 'readme.md', 'changelog.md' ].includes(filename))
    return <SiMarkdown className='text-blue-700' />;
  ...
  case 'py':
    return <SiPython className='text-yellow-600' />;
  case 'json':
  ...
  case 'md':
  case 'mdx':
    return <SiMarkdown className='text-gray-600 dark:text-gray-400' />;
  default:
    return <VscFile className='text-gray-500' />;
  }
};
