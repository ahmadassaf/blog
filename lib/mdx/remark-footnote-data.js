...
export default function remarkFootnoteData() {
  return (tree, file) => {
    ...
    // Store footnotes in file data for use by rehype plugins
    if (!file.data.footnotes) file.data.footnotes = {};

    Object.assign(file.data.footnotes, footnotes);

    // Also add footnote content directly to footnote references
    visit(tree, 'footnoteReference', (node) => {
      const id = node.identifier;

      if (footnotes[id]) {

        // Add the content as data on the node
        node.data = node.data || {};
        node.data.footnoteContent = footnotes[id];
      }
    });
  };
}
