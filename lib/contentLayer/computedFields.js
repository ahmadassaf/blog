 ...
 */
/*
 * Path of the document relative to its content root (drops the leading
 * `blog/` segment of the flattened path).
 */
const relPath = (doc) => doc._raw.flattenedPath.replace(/^[^/]*\//, '');
...
const computedFields = {
   ...
   */
  'slug': {
    'resolve': (doc) => `category/${relPath(doc)}`,
    'type': 'string'
  },
