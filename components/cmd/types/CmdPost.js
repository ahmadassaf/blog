...
function CmdPost({ title, category }) {
  ...
  return (
    <div className='contents w-full'>
      ...
      <span className='text-xs inline-flex items-center rounded-md bg-green-600 px-2 py-1 hover:bg-green-700 text-white font-medium transition-colors duration-200'>{ category.replace('-', ' ') }</span>
    </div>
  );
}
