...
<Callout type="info">[SPARQL.Anything](https://github.com/SPARQL-Anything/sparql.anything) is a system for Semantic Web re-engineering that allows users to query anything with SPARQL. In [their video about streamlining Knowledge Graph Construction with a façade: the SPARQL Anything project](https://www.youtube.com/watch?v=Ak3bykN2dgI), Enrico Daga one of the key SPARQL Anything developers gives some good background on the philosophy of Facade-X. To summarize, it models things as lists of lists. Blank nodes, play a large role in this system. </Callout>
