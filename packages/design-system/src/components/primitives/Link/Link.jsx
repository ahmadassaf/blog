...
export const linkVariants = createVariants({
  ...
  'compoundVariants': [
    ...
    { 'className': 'text-neutral-700 hover:text-neutral-900 dark:text-neutral-300 dark:hover:text-neutral-100', 'tone': 'neutral', 'variant': 'inline' },
...
const CustomLink = ({ children, className, href, prefetch = true, tone, variant, ...rest }) => {
  const isAnchorLink = href && href.startsWith('#');
  const isInternalLink = href && href.startsWith('/');
  const classes = linkVariants({ className, tone, variant });

  if (!href) return <span className={ classes } { ...rest }>{children}</span>;

  if (isInternalLink) return <NextLink href={ href } prefetch={ prefetch } className={ classes } { ...rest }>{children}</NextLink>;

  if (isAnchorLink) return <a href={ href } className={ classes } { ...rest }>{children}</a>;

  return <a className={ cn('no-underline', classes) } target='_blank' rel='noopener noreferrer' href={ href } { ...rest }>{children}</a>;
};
